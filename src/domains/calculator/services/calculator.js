 ...
 * Orchestrates the calculation flow using domain layers
 * 
 * Flow: API → calculateBestCardForExpenses() → repositories → evaluators → formatter
 */

import { findAllCards, findCardsByIds } from '../../offers/repositories/cardsRepository'
...
import { estimateOfferValue, filterOffersForCard } from './offerEvaluator'
...
export async function calculateBestCardForExpenses(input) {
  ...
  const results = cards.map(card => {
    ...
    // Evaluator: choose best rule for card
    const rule = chooseBestRule(rules, cardId)

    // Evaluator: check min_spend
    ...
    // Evaluator: calculate reward
    const rewardCalc = calculateReward(rule, amount)

    // Evaluator: filter and calc offers
